...
def handle_client_data():
    ...
    try:
        ...
        b_response = requests.post("http://server_b:5001/calculate_bmi", json={"weight": weight, "height": height}, timeout=5)
        b_response.raise_for_status()
        bmi = b_response.json().get("bmi")

        # --- Call Server C (Category Classification) ---
        c_response = requests.post("http://server_c:5002/bmi_category", json={"bmi": bmi}, timeout=5)
        c_response.raise_for_status()
        category = c_response.json().get("category")

        # --- Call Server D (Health Recommendation) ---
        d_response = requests.post("http://server_d:5003/recommendation", json={"category": category, "age": age}, timeout=5)
